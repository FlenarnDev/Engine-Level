		...
		std::vector<DialogueOption> GetDialogueOptions()
		{
			...
			if (BGSSceneActionPlayerDialogue* playerDialogue = GetCurrentPlayerDialogueAction())
			{
				...
				for (DialogueEntry infoEntry : viableInfos)
				{
					...
					option.isBarterOption = npcResponseInfo ? Shared::HasVMScript(npcResponseInfo, "VendorInfoScript") : false;
					option.isInventoryOption = npcResponseInfo ? Shared::HasVMScript(npcResponseInfo, "OpenInventoryInfoScript") : false;
