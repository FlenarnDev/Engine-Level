#pragma once

#include "Shared/SharedDeclarations.h"

namespace ObScript
{
	class ToggleArmorDegradation
	{
	public:
		static void Install()
		{
			const auto functions = RE::SCRIPT_FUNCTION::GetConsoleFunctions();
			const auto it = std::find_if(
				functions.begin(),
				functions.end(),
				[&](auto&& a_elem)
				{
					return _stricmp(a_elem.functionName, "CaptureMessage") == 0;
				});

			if (it != functions.end())
			{
				*it = RE::SCRIPT_FUNCTION{ LONG_NAME.data(), SHORT_NAME.data(), it->output };
				it->helpString = HelpString().data();
				it->executeFunction = Execute;
				
				REX::DEBUG("Registered 'ToggleArmorDegradation' command.");
			}
			else
			{
				REX::DEBUG("WARNING: Failed to register 'ToggleArmorDegradation' command.");
			}
		}

	private:
		static bool Execute(
			const RE::SCRIPT_PARAMETER*,
			const char*,
			RE::TESObjectREFR*,
			RE::TESObjectREFR*,
			RE::Script*,
			RE::ScriptLocals*,
			float&,
			std::uint32_t&)
		{
			RE::Cascadia::Shared::noArmorDegradation = !RE::Cascadia::Shared::noArmorDegradation;
			if (RE::Cascadia::Shared::noArmorDegradation)
			{
				RE::ConsoleLog::GetSingleton()->PrintLine("Armor degradation disabled.");
			}
			else
			{
				RE::ConsoleLog::GetSingleton()->PrintLine("Armor degradation enabled.");
			}
			return true;
		}

		[[nodiscard]] static const std::string& HelpString()
		{
			static auto help = []()
				{
					std::string buf;
					buf += "Toggle armor degradation"sv;
					return buf;
				}();
			return help;
		}

		static constexpr auto LONG_NAME = "ToggleArmorDegradation"sv;
		static constexpr auto SHORT_NAME = "tad"sv;
	};
}
